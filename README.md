...
在子程序引用某模块时，mewbot会在mew_modules目录下搜索同名文件夹，并通过require进行加载。
...
服务是在系统进入运行状态后，系统会加载并初始化的程序。

用户可以通过配置文件或者环境变量中设置： MEWBOT_SERVICE 来定义系统需要运行的服务。亦或者通过命令行参数(--service)来指定。

如果通过环境变量指定时，需要运行多项服务，则要用逗号隔开。

在系统进入运行状态后，会尝试在mew_modules目录下搜索以!开头的服务程序模块，如某服务为watchdog，那么其对应的服务程序模块文件夹则为：mew_modules/!watchdog。找到后则加载，值得注意的是所有服务的初始类必须继承： Mew.Service，改为引用了mew模块(require("mew")，详见5.7 mew模块)后的子成员：Service获得的。

服务在系统进入运行状态后，将会调用服务实例的start方法来启动该服务，由此在服务中可以预先定义和加载相关程序。

### 2.3.1 服务的配置

所有服务的配置文件都是写在/var/conf 下，以!${service_name}方式命名，json格式存储的。不同于消息适配器，服务通常全局只能存在一个实例，因此其配置文件也是唯一的。

服务模块也可以通过rpc来调用其configure方法来配置，详见(2.3.4 服务的rpc)

### 2.3.2 服务的方法

上述提到了关于Mew.Service父类，其中定义了服务的一些基本方法(可重载)，比较重要的方法如下：

start: 这是mewbot在正式启动后，系统会执行的方法，其附带一个参数，callback(err)，表示当前服务如果初始运行完成或出错后的回调。

afterConfigUpdate: 这个是在系统通过configure接口进行配置后，会调用的接口，表示配置完成后的回调，此时服务子程序可以执行更新配置相关操作，之后调用其第一个参数的回调函数，即可完成配置写入，如果在回调过程中遇上错误，那么该次的配置更改不会写入系统。

### 2.3.3 如何撰写服务模块

撰写服务模块首先你需要在mew_modules中准备好以这种格式(!{module name})命名的文件夹，并在其中准备好志向主类的package.json。

在撰写主类的时候，需要让它继承Mew.Service(Mew = require 'mew' )，之后可以通过重载start等方法来实现对服务的定义。

服务的构造函数有两个参数，第一个为@mew,即mewbot的实例，第二个为@config，表示该服务当前所保存的配置。

### 2.3.4 服务的rpc

服务会向系统的rpc服务栈(关于rpc机制，详见4 rpc机制)注册以自己服务名命名的rpc对象，其会包装所有的服务顶层方法作为rpc可访问对象。因此在设计服务的顶层方法时，必须将一个回调函数作为最后一个参数，并且该回调函数接收两个参数(err,result)，分别表示返回的错误和结果。

# 3 消息机制

Mewbot设计的初衷是为了根据规则自动处理并dispatch消息，简单的可以作为聊天室的聊天机器人，亦或者
